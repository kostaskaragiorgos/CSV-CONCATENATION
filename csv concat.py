...
class CSV_CONCATENATION():
    def __init__(self,master):
        ...
        self.file_menu.add_command(label = "Show_List",command = self.showlista)
        self.file_menu.add_command(label="Exit",accelerator= 'Alt+F4',command = self.exitmenu)
        self.menu.add_cascade(label = "File",menu=self.file_menu)
        
        self.Edit_menu = Menu(self.menu,tearoff = 0)
        self.Edit_menu.add_command(label = "Clear list",accelerator = 'Ctrl+Z',command = self.clearl)
        ...
        self.master.bind('<Control-z>',lambda event:self.clearl())
